...
Si pasa la propiedad `thedata-path` la pestaña hará un
`routing.replaceState` en el valor pasado a la
`thedata-path`.
...
If you pass the `data-path` property over the tabs
each tab will do a `routing.replaceState` on the value
passed to data-path. tab will do a routing.replaceState on the
value passed to `data-path`.
