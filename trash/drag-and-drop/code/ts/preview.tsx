...
export function Preview(): JSX.Element {
	...
	const deleteFile = (event: MouseEvent<HTMLSpanElement>): void => {
		...
		const fileFilter: File[] = files.filter((file: File): boolean => file.name !== dataset.name);
	...
	const preview: JSX.Element[] = files.map(
		(item: File, index: number): JSX.Element => (
			<div key={index} className="drop-file-preview__item">
				...
				<span data-name={item.name} onClick={deleteFile} className="drop-file-preview__item__del">
					x
				</span>
			</div>
		)
		...
		<>
			...
			{!!files.length && (
				<div className="drop-file-preview">
					...
					<div className="drop-file-preview__content">{preview}</div>
				</div>
			)}
		</>
	);
