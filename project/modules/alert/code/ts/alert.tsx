...
function Alert(props: IProps) {
	const { message, show, className, mode, title, isClose, children } = props;
	...
	const cls: string = className ? `${className} alert ${mode ?? 'success'}` : `alert ${mode ?? 'success'}`;
		...
		<div className={cls} onClick={close}>
			...
			{title && (
				<div className='alert__title'>
					...
					<span className='alert__title-span' dangerouslySetInnerHTML={{ __html: title }} />
				</div>
			)}
			{children}
