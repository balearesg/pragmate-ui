...
export function Area(): JSX.Element {
  const { files, count, formats, showMessage, state, dispatch, disabled } = useDragAndDropContext();
  const accept: string[] = formats && Array.isArray(formats) && formats.map(item => `.${item}`);
  const cls: string = count && files.length < count && !disabled ? 'hover FilesDragAndDrop__area' : 'FilesDragAndDrop__area';
  ...
  return (
    <div className={cls}>
      ...
      {count && files.length < count && !disabled && <input {...attrs} type="file" onChange={onFileDrop} />}
