...
export function Imports({ children }: { children: ReactNode }): JSX.Element {
	const { state, count, drop, drag, formats, dispatch, onUpload, clean, alert } = useDragAndDropContext();
	...
	const textAlertInfo: string = `${alert ? `${alert} <br/>` : ""} ${count && count !== Infinity ? `Puede cargar un máximo de ${count} archivo${count !== 1 ? 's' : ''}` : ''
		} ${textFormats}`;
	...
	return (
		<Form className="drag-and-drop-container" onSubmit={handleSubmit}>
			...
			<Alert type="info">
				<p className="alert__title-span" dangerouslySetInnerHTML={{ __html: textAlertInfo }} />
			</Alert>
